...
import { useWalletModal } from '@solana/wallet-adapter-react-ui';  // Add this for modal control
...
export default function Minimal() {
  ...
  const { setVisible } = useWalletModal();  // For opening wallet selection modal
  ...
  const handleWalletClick = () => {
    if (connected) {
      ...
      setVisible(true);  // Open modal to select/connect wallet
    }
  };
  ...
  return (
    <div suppressHydrationWarning={true} className="font-sans grid grid-rows-[1fr_20px] items-start justify-items-center min-h-screen px-4 pb-4 gap-8 sm:px-10 sm:pb-10 text-xl text-[#00ff00] bg-transparent relative">
      <main className="flex flex-col items-center justify-center w-full max-w-2xl mx-auto p-1 sm:p-10">
        ...
        <div className="fixed top-0 left-0 w-1/3 p-8 rounded-lg shadow-md shadow-green-500/50 bg-black/50 z-10">
          {/* Header section */}
